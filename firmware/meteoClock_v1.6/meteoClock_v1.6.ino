...
/* Версия 1.6
  - Добавлена поправка температуры (TEMP_CORRECTION) по эталонным термометрам
  - Графики переведены на однобайтовое хранение с масштабированием (TEMP_K/PRESS_K/RAIN_K) для экономии RAM
  - Команды датчика СО2 (MH-Z19) перенесены в PROGMEM для экономии RAM
  - Удалены неиспользуемые массивы графиков дождя (rainHour/rainDay)
  - Итог: RAM 1705 -> 1475 байт, свободно 573 байта (было 343)
*/
...
#define TEMP_CORRECTION -0.6        // поправка температуры (°C): показания станции - среднее эталонных термометров
Adafruit_BME280 bme;
...
// массивы графиков (byte + коэффициент, чтобы показания влезли в байт) (с)НР
#define TEMP_K 40                 //поправочный коэффициент, чтобы показания влезли в байт
#define PRESS_K -600              //поправочный коэффициент, чтобы показания влезли в байт
#define RAIN_K 100                //поправочный коэффициент, чтобы показания влезли в байт
byte tempHour[15], tempDay[15];
byte humHour[15], humDay[15];
byte pressHour[15], pressDay[15];
byte co2Hour[15], co2Day[15];     // СО2 делится на 10
byte altHour[15], altDay[15];     // высота делится на 10
int delta;
...
void drawPlot(byte pos, byte row, byte width, byte height, int min_val, int max_val, const byte *plot_array, const char* label1, const char* label2, int stretch, int mul, int add) {  // график ---------------------------------
  int max_value = -32000;
  int min_value = 32000;

  for (byte i = 0; i < 15; i++) {
    int v = (int)plot_array[i] * mul + add;
  ...
  lcd.setCursor(16, 2); lcd.print((int)plot_array[14] * mul + add);
  ...
  for (byte i = 0; i < width; i++) {                  // каждый столбец параметров
    ...
    if (v > min_val)
      infill = floor((float)(v - min_val) / (max_val - min_val) * height * 10);
    else infill = 0;
...
void setup() {
  ...
  for (byte i = 0; i < 15; i++) {   // счётчик от 0 до 14
    tempHour[i] = (byte)(dispTemp + TEMP_K);
    tempDay[i] = (byte)(dispTemp + TEMP_K);
    humHour[i] = dispHum;
    humDay[i] = dispHum;
    altHour[i] = (byte)(dispAlt / 10);
    altDay[i] = (byte)(dispAlt / 10);
    if (PRESSURE) {
      pressHour[i] = (byte)(0 + RAIN_K);
      pressDay[i] = (byte)(0 + RAIN_K);
    } else {
      pressHour[i] = (byte)(dispPres + PRESS_K);
      pressDay[i] = (byte)(dispPres + PRESS_K);
    }
